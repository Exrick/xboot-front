...
router.beforeEach((to, from, next) => {
    ...
    if (Cookies.get('locking') === '1' && to.name !== 'locking') { // 判断当前是否是锁定状态
        ...
        if (!Cookies.get('userInfo') && to.name !== 'login' && to.name !== 'regist' && to.name !== 'regist-result') { // 判断是否已经登录且前往的页面不是登录页
        ...
        } else if (Cookies.get('userInfo') && to.name === 'login') { // 判断是否已经登录且前往的是登录页
            Util.title();
            next({
                name: 'home_index'
            });
            ...
            if (curRouterObj && curRouterObj.access !== undefined && curRouterObj.access !== null) { // 需要判断权限的路由
                let accessCode = Cookies.get('access');
                if (Util.showThisRoute(curRouterObj.access, accessCode)) {
                    Util.toDefaultPage([otherRouter, ...store.state.app.menuList], to.name, router, next); // 如果在地址栏输入的是一级菜单则默认打开其第一个二级菜单的页面
                } else {
                    next({
                        replace: true,
                        name: 'error-403'
